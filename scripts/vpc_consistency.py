...
from nxapi_netbox.nxapi.nxapi_vpc_consistency import NxapiVpcConsistencyGlobal, NxapiVpcConsistencyVni, NxapiVpcConsistencyVlans, NxapiVpcConsistencyInterface
...
def get_parser():
    ...
    default.add_argument('--mismatched_labels',
    ...
    default.add_argument('--interfaces',
                        dest='interfaces',
                        required=False,
                        default=None,
...
def show_inconsistent_params(ip, nx, interface=None):
    ...
    if len(inconsistent_items) == 0:
        if interface == None:
            lines.append("{:<15} {:<20} all {} {} vpc params are consistent".format(ip, nx.hostname, len(nx.info), nx.param_type))
        else:
            lines.append("{:<15} {:<20} {} all {} {} vpc port-channel params are consistent".format(ip, nx.hostname, interface, len(nx.info), nx.param_type))
    else:
        for item in nx.inconsistent_params:
            if interface == None:
                lines.append('{:<15} {:<20} {}'.format(ip, nx.hostname, item['vpc-param-name']))
            else:
                lines.append('{:<15} {:<20} {} {}'.format(ip, nx.hostname, interface, item['vpc-param-name']))
            for key in item:
...
def show_mismatched_labels(ip, nx):
    ...
    if len(nx.mismatched_info) > 0:
        for label in nx.mismatched_info:
            lines.append('{:<15} {:<20} vpc-param-name {}'.format(ip, nx.hostname, label))
...
def worker(device, vault):
    ...
    lines = list()
    for class_name in [NxapiVpcConsistencyGlobal, NxapiVpcConsistencyVni, NxapiVpcConsistencyVlans]:
    ...
    for interface in cfg.interfaces.split(','):
        nx = NxapiVpcConsistencyInterface(vault.nxos_username, vault.nxos_password, ip, log)
        nx.nxapi_init(cfg)
