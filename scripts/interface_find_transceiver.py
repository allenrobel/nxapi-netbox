...
def get_parser():
    ...
    default.add_argument('--nom_bitrate',
                        dest='nom_bitrate',
                        required=False,
                        default=-1,
                        help='{} {}'.format(help_nom_bitrate, ex_nom_bitrate))
    default.add_argument('--partnum',
                        dest='partnum',
                        required=False,
                        default='xxxxxxxxxxxxxxxx',
                        help='{} {}'.format(help_partnum, ex_partnum))
    default.add_argument('--rev',
                        dest='rev',
                        required=False,
                        default='xxxxxxxxxxxxxxxx',
                        help='{} {}'.format(help_rev, ex_rev))
    default.add_argument('--serialnum',
                        dest='serialnum',
                        required=False,
                        default='xxxxxxxxxxxxxxxx',
...
def print_header():
    print(fmt.format('ip', 'hostname', 'interface', 'serial_num', 'search_term', 'value'))

def search(ip, nx):
    lines = list()
    if cfg.ciscoid in nx.ciscoid:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'ciscoid', nx.ciscoid))
    if cfg.ciscoid_1 in nx.ciscoid_1:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'ciscoid_1', nx.ciscoid_1))
    if cfg.cisco_part_number in nx.cisco_part_number:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'cisco_part_num', nx.cisco_part_number))
    if cfg.cisco_product_id in nx.cisco_product_id:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'product_id', nx.cisco_product_id))
    if cfg.cisco_vendor_id in nx.cisco_vendor_id:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'vendor_id', nx.cisco_vendor_id))
    if cfg.name in nx.name:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'name', nx.name))
    try:
        cfg_nom_bitrate = int(cfg.nom_bitrate)
    except:
        log.error('exiting. expected int() for --nom_bitrate, got {}'.format(cfg.nom_bitrate))
        exit(1)
    if cfg_nom_bitrate >= nx.nom_bitrate:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'nom_bitrate', nx.nom_bitrate))
    if cfg.partnum in nx.partnum:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'partnum', nx.partnum))
    if cfg.rev in nx.rev:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'rev', nx.rev))
    if cfg.serialnum in nx.serialnum:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'serialnum', nx.serialnum))
    if cfg.type in nx.sfp_type:
        lines.append(fmt.format(ip, nx.hostname, nx.interface, nx.serialnum, 'type', nx.sfp_type))
    return lines
