...
def get_parser():
    ...
    help_state = (
        "if specified, display neighbors with matching state.  Valid values: {}".format(
            ",".join(sorted(str(valid_bgp_state)))
        )
    )
...
def collect_output(ip, nx, lines):
    ...
    lines.append(
        fmt.format(ip, nx.hostname, "neighbortableversion", nx.neighbortableversion)
    )
...
def show_bgp_neighbors_l2vpn_evpn(ip, nx, state=None):
    ...
    for neighbor in nx.neighbor_info:
        ...
        try:
            ...
            log.error(
                "exiting. cannot convert nx.prefixreceived {} to int()".format(
                    nx.prefixreceived
                )
            )
            exit(1)
