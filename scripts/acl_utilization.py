...
from nxapi_netbox.nxapi.nxapi_system_internal_access_list_resource_utilization import (
    NxapiAccessListResourceUtilization,
)


def get_parser():
    ex_prefix = "Example:"
    help_type = "Type of information to retrieve. One of {}".format(
        ",".join(valid_types)
    )
...
def print_header():
    if "all" in cfg.type:
        print(
            "{:<15} {:<20} {:>11} {:>6} {:>8} {:<36}".format(
                "ip", "hostname", "{}".format(cfg.type), "module", "instance", "feature"
            )
        )
    else:
        print(
            "{:<15} {:<20} {:>11} {:>6} {:<36}".format(
                "ip", "hostname", "{}".format(cfg.type), "module", "feature"
            )
        )
...
def is_within_threshold(item):
    try:
        ...
        log.error(
            "Could not convert item {} to float.  Displaying it regardless.".format(
                item
            )
        )
        return True
...
def get_all_type(nx, ip, item):
    ...
    for instance in item:
        ...
        lines.append(
            "{:<15} {:<20} {:>11} {:>6} {:>8} {:<36}".format(
                ip, nx.hostname, item[instance], nx.module, instance, nx.feature
            )
        )
    return lines
...
def get_max_min_type(nx, ip, item):
    ...
    lines.append(
        "{:<15} {:<20} {:>11} {:>6} {:<36}".format(
            ip, nx.hostname, item, nx.module, nx.feature
        )
    )
    return lines
...
def worker(device, vault, modules):
    ...
    nx = NxapiAccessListResourceUtilization(
...
if cfg.type not in valid_types:
    log.error(
        "Exiting. Invalid type {}.  Expected one of {}".format(
            cfg.type, ",".join(valid_types)
        )
    )
    exit(1)
try:
    ...
    log.error(
        "Exiting. Could not convert --ge {} or --le {} to float: {}".format(
            cfg.ge, cfg.le
        )
    )
    exit(1)
